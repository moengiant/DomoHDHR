...
for device in tunerIDs:
    ...
    if channels[titleNumber] != 'none':
        ...
        lookup = 'Channel' + channels[titleNumber] + ';Channel' + channels[titleNumber] + ';Channel' + channels[titleNumber] + ' Logo'
        print(lookup)
        #Get CustomImage ID (row number in switch_icons.txt file) for tv channel
        with open('../../www/switch_icons.txt') as myFile:
            for num, line in enumerate(myFile, 0):
                if lookup in line:
                    print ('CustomImage ID: ' + str(num))
                    ci = str(num)
        
        renameDevice = 'http://192.168.1.2:8080/json.htm?type=setused&used=true&idx=' + device + '&switchtype=0&name=HDHomerun Tuner ' + str(titleNumber) + ' - Channel ' + channels[titleNumber] + '&customimage=' + ci + ''
        print(renameDevice)
